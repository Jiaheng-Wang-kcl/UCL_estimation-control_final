...
class SimulatorConfiguration(object):
    def __init__(self):
        self.dt = 0.001 # changed by Jiaheng
...
class Simulator(object):
    ...
    def generate_range_bearing_observations(self):
        y = []
        ...
        for lm in self._map.landmarks:
            ...
            bearing_meas = np.arctan2(dy, dx) - self._x_true[2] + np.random.normal(0, np.sqrt(W_bearing))
...
simulator.start()
print("test simulator start",simulator._x_true)
...
# Extract the initial estimates from the filter and use these to generate the control for the first timestep.
x_est, Sigma_est = estimator.estimate()
print("test estimator start",estimator._x_est,estimator._Sigma_est)
...
for step in range(sim_config.time_steps):
    ...
    # print("test simulator",simulator._x_true)
    print("check simulation_time",simulation_time)
    ...
    # print("test estimator",estimator._x_est)

    # Get the landmark observations.
    # y = simulator.landmark_range_observations()
    y = simulator.generate_range_bearing_observations()
...
estimation_error[:, -1] = wrap_angle(estimation_error[:, -1])  # Handle angle wrapping for theta

# Compute and print mean and variance of errors
state_name = ['x', 'y', 'θ']
for s in range(3):
    mean_error = np.mean(estimation_error[:, s])
    var_error = np.var(estimation_error[:, s])
    print(f"{state_name[s]} estimation error (dt = {sim_config.dt}):")
...
plt.title(f"Unicycle Robot Localization using EKF (dt = {sim_config.dt})")
plt.axis('equal')
plt.grid(True)
plt.show()
...
for s in range(3):
    ...
    plt.plot(two_sigma, linestyle='dashed', color='red', label="2σ upper")
    plt.plot(-two_sigma, linestyle='dashed', color='red', label="2σ lower")
    plt.title(f"{state_name[s]} estimation error (dt = {sim_config.dt})")
    plt.legend()
    plt.show()
